    ...
    wordList.forEach(
        word -> {
          for (int i = 0; i < L; i++) {
            ...
            String newWord = word.substring(0, i) + '*' + word.substring(i + 1, L);
            System.out.println("new word " +newWord+" "+word);
      ...
      return 0;
        ...
        while(!queue.isEmpty()) {
            ...
            for(int i=0;i<wordList.size();i++) {
                if(visited[i] != 1) {
                    if(findDiff(str, wordList.get(i)) == 1) {
                        queue.add(new Pair(pr.level, wordList.get(i)));
                        visited[i] = 1;
                        if(wordList.get(i).equals(endWord)) {
                            minTransform = Math.min(minTransform, pr.level);
                            return minTransform;
                        }
                    }            
                }
